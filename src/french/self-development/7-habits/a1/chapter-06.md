# Habitude 6 — Créez une synergie 🟢 A1

> *« La synergie, c'est 1 + 1 = 3 ou plus. »*
> — Stephen R. Covey

---

## 📖 Résumé

Vous avez deux personnes. Ensemble, elles font quelque chose. Le résultat est meilleur que ce que chaque personne peut faire seule. C'est la <span class="gloss" data-def="synergy — together you create more than alone">**synergie**</span>.

La sixième habitude dit : travaillez ensemble. Respectez les <span class="gloss" data-def="differences — the ways people are not the same">**différences**</span>. Créez quelque chose de mieux.

Covey donne un exemple simple. Deux plantes. Seules, elles ne <span class="gloss" data-def="to grow — to get bigger and stronger">poussent</span> pas bien. Mises ensemble dans un bon <span class="gloss" data-def="soil — the earth where plants grow">sol</span>, elles poussent beaucoup mieux. La synergie crée quelque chose de nouveau et de meilleur.

Beaucoup de personnes ont peur des différences. Elles préfèrent les personnes <span class="gloss" data-def="similar — almost the same as you">similaires</span>. Elles disent : « Je suis d'accord » même quand ce n'est pas vrai.

Les personnes qui comprennent la synergie font le contraire. Elles **cherchent** les différences. Elles pensent : « Cette personne voit les choses différemment de moi. Peut-être qu'elle a raison. Peut-être que nous pouvons trouver une meilleure solution ensemble. »

La synergie est possible seulement quand vous avez :
- L'**Habitude 4** : Penser Gagnant-Gagnant
- L'**Habitude 5** : Écouter avec empathie

Avec ces trois habitudes ensemble (4 + 5 + 6), vous pouvez créer des <span class="gloss" data-def="teams — groups of people working together">équipes</span> et des relations extraordinaires.

**Le message principal :** Le tout est plus grand que la somme des parties. Cherchez la <span class="gloss" data-def="collaboration — working together toward a shared goal">collaboration</span>, pas la <span class="gloss" data-def="competition — trying to beat others instead of joining them">compétition</span>.

---

## 🔤 Vocabulaire

| Mot | Signification |
|:----|:-------------|
| **la synergie** | synergy — when together is better than alone |
| **ensemble** | together |
| **seul / seule** | alone |
| **la différence** | difference |
| **respecter** | to respect |
| **pousser** | to grow (plants), to push |
| **le sol** | soil, ground |
| **similaire** | similar |
| **la collaboration** | collaboration, working together |
| **la compétition** | competition |
| **extraordinaire** | extraordinary, exceptional |
| **la somme** | sum, total |

---

## 💬 Expressions & Idiomes

| Expression | Signification |
|:-----------|:-------------|
| **« l'union fait la force »** | unity is strength (French proverb) |
| **« deux têtes valent mieux qu'une »** | two heads are better than one |
| **« mettre en commun »** | to pool together, to share resources |
| **« tirer dans le même sens »** | to pull in the same direction, to work toward the same goal |
| **« plus on est de fous, plus on rit »** | the more the merrier |

---

## 🧠 Grammaire — Les nombres et les opérations

Les nombres et les expressions mathématiques en français :

**Nombres cardinaux (1–20) :**

| 1 un | 2 deux | 3 trois | 4 quatre | 5 cinq |
|:-----|:-------|:--------|:---------|:-------|
| 6 six | 7 sept | 8 huit | 9 neuf | 10 dix |
| 11 onze | 12 douze | 13 treize | 14 quatorze | 15 quinze |
| 16 seize | 17 dix-sept | 18 dix-huit | 19 dix-neuf | 20 vingt |

**Les opérations :**

| Symbole | Expression | Exemple |
|:--------|:-----------|:--------|
| + | plus | Un **plus** un **égale** deux. |
| - | moins | Cinq **moins** deux **égale** trois. |
| × | fois | Deux **fois** trois **égale** six. |
| = | égale | La synergie : un **plus** un **égale** trois ! |

**Ordinal numbers (position) :**

| 1er/1ère | 2e | 3e | 4e | 5e | 6e | 7e |
|:---------|:---|:---|:---|:---|:---|:---|
| premier/première | deuxième | troisième | quatrième | cinquième | sixième | septième |

**Exemples du chapitre :**
- C'est la **sixième** habitude.
- **Un** plus **un** égale **trois** avec la synergie.
- **Deux** plantes ensemble poussent **trois** fois mieux.

---

## ✍️ Exercices

**1. Vrai ou faux ?**

a) La synergie signifie que 1 + 1 = 2.
b) Les personnes synergiques cherchent les différences.
c) Pour créer la synergie, vous avez besoin des habitudes 4 et 5 aussi.
d) Il faut avoir peur des personnes différentes.

*Réponses : a) F — b) V — c) V — d) F*

---

**2. Complétez avec le bon nombre**

a) Un plus deux égale _______.
b) Cinq moins trois égale _______.
c) Deux fois quatre égale _______.
d) C'est la _______ habitude de Covey. (6 en ordinal)

*Réponses : a) trois — b) deux — c) huit — d) sixième*

---

**3. Synergie ou compétition ?**

Lisez la situation. Écrivez **S** (synergie) ou **C** (compétition).

a) Deux collègues partagent leurs idées et trouvent une solution créative. → ___
b) Deux amis veulent le même prix. Un seul peut gagner. → ___
c) Une équipe combine ses différentes expertises pour un projet. → ___
d) Deux voisins ne se parlent pas et font le même travail séparément. → ___

*Réponses : a) S — b) C — c) S — d) ni S ni C (juste séparé)*

---

**4. Traduction**

a) Together, we are stronger.
b) Two heads are better than one.
c) Respect the differences.

*Exemples : a) Ensemble, nous sommes plus forts. — b) Deux têtes valent mieux qu'une. — c) Respectez les différences.*

---

## 📝 Glossaire

- **la partie** : part (« la somme des parties » = the sum of the parts)
- **le tout** : the whole
- **l'expertise** (f.) : specialized knowledge or skill
- **créatif / créative** : creative
- **la plante** : plant (flora)

---

## 🔊 Audio

▶️ [Écouter le chapitre 6 — A1](../audio/a1-chapter-06.mp3)

*Conseil : Pensez à un exemple de synergie dans votre vie. Racontez-le en quelques phrases simples en français.*

---

<div style="display:flex; justify-content:space-between; margin-top:3em;">

[← Habitude 5](chapter-05.md) &nbsp;&nbsp;&nbsp; [Habitude 7 →](chapter-07.md)

</div>
